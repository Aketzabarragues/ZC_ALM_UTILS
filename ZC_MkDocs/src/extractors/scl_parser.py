...
def limpiar_markdown(texto: str) -> str:
    """Calcula la indentación base de TIA Portal y la elimina de todas las líneas."""
    if not texto: return ""
    texto = texto.expandtabs(4)
    lineas = texto.splitlines()
    espacios = [len(l) - len(l.lstrip()) for l in lineas if l.strip()]
    min_espacios = min(espacios) if espacios else 0
    lineas_limpias = [l[min_espacios:] if len(l) >= min_espacios else l for l in lineas]
...
def parsear_scl(ruta_archivo: Path) -> SiemensBlock:
    ...
    if match_region:
        ...
        # 1. Información del Sistema (búsqueda独立性)
        ...
        # 2. Restricciones (búsqueda independiente con limpieza)
        rest_match = re.search(r'>\s*\*\*Restricciones:\*\*(.*?)(?=\n\s*---|###|\*\))', header_text, re.DOTALL | re.IGNORECASE)
        if rest_match:
            restrictions = limpiar_markdown(rest_match.group(1))

        # 3. Descripción Funcional (con limpieza)
        desc_match = re.search(r'###\s*Descripción Funcional\s*(.*?)(?=\n\s*---|###|\*\))', header_text, re.DOTALL | re.IGNORECASE)
        if desc_match:
            description = limpiar_markdown(desc_match.group(1))
